...
const TableSkeleton = () => (
  <div className="animate-pulse p-6">
    {Array.from({ length: 5 }).map((_, i) => (
      <div key={i} className="flex items-center space-x-4 py-4 border-b border-slate-200">
...
const CouponList = ({ onAdd, onEdit }) => {
  ...
  const filteredCoupons = useMemo(() => {
    ...
    if (filterType === "active") {
      filtered = filtered.filter((c) => c.is_active && !isExpired(c.expire_date));
  ...
  const statsData = useMemo(() => {
    ...
    const active = coupons.filter((c) => c.is_active && !isExpired(c.expire_date)).length;
    const totalUsage = coupons.reduce((sum, c) => sum + (c.used_count || 0), 0);
    const avgDiscount =
      coupons.length > 0
        ? (coupons.reduce((sum, c) => sum + (c.discount_percentage || 0), 0) / coupons.length).toFixed(1)
        : 0;
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          const usagePercentage = (row.original.used_count / row.original.can_used_up_to) * 100 || 0;
          ...
          const getActivityIcon = () => {
            switch (row.original.activity) {
              case "trip": return "mdi:airplane";
              case "course": return "mdi:school";
              default: return "mdi:star-circle";
            }
          };
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor()} text-white`}>
                    <div className="w-1.5 h-1.5 bg-white rounded-full"></div>
                    {!row.original.is_active ? "Inactive" : expired ? "Expired" : row.original.remaining <= 0 ? "Used Up" : usagePercentage > 80 ? "Low" : "Active"}
                  </span>
                  <span className="text-xs text-slate-500 capitalize">{row.original.activity === "all" ? "All" : row.original.activity}</span>
                </div>
              </div>
            </div>
          );
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="text-sm">
              <div className={`font-medium ${expired ? "text-red-600" : "text-slate-800"}`}>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            ...
            <button
              onClick={onAdd}
              className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-8 py-3.5 rounded-xl font-semibold hover:shadow-2xl transform hover:scale-105 transition-all duration-200 flex items-center space-x-2 whitespace-nowrap"
            >
