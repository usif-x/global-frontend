...
const GeniusStatCard = ({ title, value, subValue, icon, color, trend, trendValue }) => (
    ...
    <div className="flex justify-between items-start mb-4">
      ...
      {(trend || trendValue) && (
        <div className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${
          ...
          <Icon icon={trend === 'up' ? "mdi:trending-up" : "mdi:trending-down"} />
          {trendValue}
        </div>
      )}
    </div>
...
const QuickActionBtn = ({ icon, title, desc, onClick }) => (
  ...
  >
    <div className="p-3 bg-slate-50 rounded-lg group-hover:bg-white transition-colors">
      <Icon icon={icon} className="w-6 h-6 text-slate-500 group-hover:text-cyan-600" />
    </div>
    <div className="ml-4">
      <h4 className="font-semibold text-slate-700 group-hover:text-cyan-800">{title}</h4>
      <p className="text-xs text-slate-500">{desc}</p>
    </div>
    <Icon icon="mdi:chevron-right" className="w-5 h-5 text-slate-300 ml-auto group-hover:text-cyan-400" />
  </button>
);
...
const HeroDashboard = ({ setActiveTab, admin }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [analytics, recentUsersData] = await Promise.all([
      ...
      const notifs = [];
      if (analytics.stats?.pending_invoices_today > 0) {
        notifs.push({
          id: 1,
          type: 'warning',
          message: `${analytics.stats.pending_invoices_today} invoices need review`,
          time: 'Today'
        });
      }
  ...
  const { stats, top_customers, recent_transactions, recentUsers } = dashboardData || {};
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 shadow-xl text-white">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="text-4xl font-bold mb-2">Hello, {admin?.name?.split(' ')[0] || 'Captain'}! 👋</h1>
          <p className="text-slate-300 text-lg mb-6">
            Here's what's happening with your business today. You have <span className="text-cyan-400 font-bold">{stats?.pending_invoices_today || 0} pending items</span> requiring attention.
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <GeniusStatCard 
          title="Revenue Today" 
          value={formatEGP(stats?.revenue_today)} 
          subValue={`${stats?.sales_count_today} Sales Made`}
          icon="mdi:cash-fast" 
        ...
        />
        <GeniusStatCard 
          title="Active Potential" 
          value={formatEGP(stats?.potential_revenue)} 
          subValue={`${stats?.pending_invoices_today} Pending Orders`}
          icon="mdi:cash-clock" 
          color="bg-amber-500"
          trend="down"
          trendValue="Pending"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setActiveTab('invoices')} className="text-sm text-cyan-600 hover:underline">View All</button>
          </div>
          <div className="space-y-4">
             {recent_transactions?.slice(0, 5).map((tx) => (
               <div key={tx.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-cyan-200 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      tx.status === 'PAID' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'
                    }`}>
                      <Icon icon={tx.status === 'PAID' ? "mdi:check" : "mdi:clock-outline"} />
                    </div>
                    <div>
                      <p className="font-semibold text-slate-800">{tx.buyer}</p>
                      <p className="text-xs text-slate-500">{new Date(tx.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} • Invoice #{tx.id}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-slate-800">{formatEGP(tx.amount)}</p>
             ...
             {(!recent_transactions || recent_transactions.length === 0) && (
               <div className="text-center py-10 text-slate-400">No recent transactions today</div>
             )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Quick Shortcuts</h3>
          ...
          {/* Top Customer Spotlight */}
           <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl shadow-lg p-6 text-white text-center relative overflow-hidden">
             <div className="relative z-10">
               <h3 className="text-sm font-semibold text-indigo-200 uppercase tracking-widest mb-4">Star Customer</h3>
               {top_customers && top_customers.length > 0 ? (
                 <>
                  ...
                  <p className="text-indigo-200 text-sm mb-4">{top_customers[0].email}</p>
                  <div className="bg-white/10 rounded-lg p-2 inline-block backdrop-blur-sm">
                    <span className="font-bold text-white">{formatEGP(top_customers[0].total_spent)}</span>
                    <span className="text-indigo-200 text-xs ml-1">Lifetime</span>
                  </div>
                 </>
               ) : (
